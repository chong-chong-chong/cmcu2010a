...
a=1.78/2; b=1.2/2; L=2.45; alpha1=0;alpha2=4.1/180*pi;
    %h1, h2 临界高度值
filename="C:\Users\xu\Documents\大学生数模竞赛\2019\国赛\2010A题\问题A附件1：实验采集数据表.xls";
...
title(ax1,'无位变和纵向位变4.1°时的理论油量');
plot(ax2,B,A, B1,A1);
title(ax2,'无位变和纵向位变4.1°时的实际油量');
c0=v0-A;c1=v1-A1;
figure
ax1=subplot(2,2,1);
plot(ax1,B,v0,B,A,'--');   %可以看出理论油量和实际油量的偏差
title(ax1,'理论油量和实际油量，无位变')
ax2=subplot(2,2,3);
...
function l=f1(h,alpha) %当油面标高为h时，垂直于h方向的柱面高度函数
   global L;
   h1=2.05*tan(alpha);
   if alpha==0
       l=L;
...
function height=f2(x,h,alpha) %当油面标高位h时，在x处的椭圆截面高度
    global b;
    if alpha==0

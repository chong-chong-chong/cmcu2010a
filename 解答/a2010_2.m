...
global R r L ;
r=1.5; R=13/8; L=8;
filename="C:\Users\xu\Documents\大学生数模竞赛\2019\国赛\2010A题\问题A附件2：实际采集数据表.xls";
...
%油位标定（以alpha=2.1°,beta=4.6°）
i=10:10:2*r*100; c=zeros(size(i));
for j=1:length(i)
    c(j)=volume(10*i(j),2.1*pi/180,4.6*pi/180);
    c1(j)=volume(10*i(j),0,0);
end
    
figure
ax1=subplot(1,1,1);
plot(ax1,a,B(I),'*',i*10,c1,'b--o',i*10,c,i*10,c1-c,'--');
title(ax1,'原油位容量、理论油量、实际容量及差值对比图：纵向倾角2.1°,横向倾角4.3°');
    

E=xlsread(filename,1,'D:D'); % 每次出油量数据

%由实际数据求alpha,beta 取第一段出油量数据 （得 alpha=2.3, beta=8.9, M=1.1638e04, 无校正； alpha=2.3 beta=8.9, 有校正， 校正可在volume函数中调整）
k=1:302;  
alpha_0=2.3;beta_0=8.9;M=0; 
...
%由实际数据求alpha,beta(取油面高度在500-1800之间的数据，误差平滑数据段）
k=111:302;  
alpha_0=2.1;beta_0=4.6;M=0; 
tmp=volume(A(k(1)),alpha_0*pi/180,beta_0*pi/180);
for s=1:size(k)-1
    tmp1=volume(A(k(s+1)),alpha_0*pi/180,beta_0*pi/180);
    M=M+(tmp-tmp1-E(k(s+1)))^2;
...
function l=length1(h,alpha,beta) %当高度为h时，垂直于h方向的柱面长度函数
   global L r;
...
function height=height1(x,h,alpha,beta) %当油面标高位h时，在x出的椭圆截面高度
    global r;
    if alpha==0
...
function v=volume1(h,alpha,beta)  %油面标高h时的理论油量
    global r;
...
function v=volume2(h,alpha,beta)
    global R r;
    h3=r-3*tan(alpha)*sec(beta);
    h2=r*(sec(beta)+1)-2*tan(alpha)*sec(beta);
...
function height=height3(x,h,alpha,beta)
    global L R r;
...
function v=volume(h,alpha,beta)  %理论+调整油量
   v=volume1(h/1000,alpha,beta)+volume2(h/1000,alpha,beta)+volume3(h/1000,alpha,beta)+f(h);
end

% 实际油量与理论油量调整  实际=理论+f(h)，f(h) 由cftool 得到
function v=f(h)
    v=1.567e-11*h.^4-6.244e-08*h.^3+9.851e-05*h.^2-0.04286*h-25.87;
end
